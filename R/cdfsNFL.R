...
nfl_total_cdf <- function(line, means, engine = "cf", n = 10^6)
{
  over <- 0
  under <- 0
  push <- 0
  ...
  if(engine == "cf")
  {
    over <- 1-plcpois(line, means[-4], point_coef, engine = "cf")
    under <- plcpois(line-1, means[-4], point_coef, engine = "cf")
    push <- dlcpois(line, means[-4], point_coef)
  }
  if(engine == "mc")
  {
